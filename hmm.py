...
class BaseIOHMM(object):
    ...
    def generate_synthetic_data(self, data):
        ...
        model_collection = [models[self.num_emissions - 1] for models in self.model_emissions]
    ...
    def predict_old_user(self, past_data, test_data):
        """
        data: past (input, output) data, new input sequence data
        1) using viterbi algorithm to estimate past state
        2) using transition probability to compute the most probable state
        3) using current state to predict user behavior
        return predictions
        """
        # initialize input data
        # assert len(past_data) == len(test_data)
